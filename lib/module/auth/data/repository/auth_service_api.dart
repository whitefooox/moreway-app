...
class AuthServiceAPI implements IAuthService {
  ...
  Future<Token> signIn(SignInData data) async {
    final signInDataModel = SignInDataModel(
      email: data.email, 
      password: data.password
    );
    try {
      final response = await _dio.post(Api.loginUrl, data: signInDataModel.toJson());
    ...
    switch (e.type) {
      case DioExceptionType.badResponse:
        switch (e.response!.statusCode){
          case 401:
          case 404:
            return const AuthException(message: "Неправильная почта или пароль");
          case 500:
